...
<div class="main">
	<div class="content users-list">
		...
		{
		?>
			<div class="msg" style="display: block"><span class="red">Video has been deleted successfully!</span></div>
		<?php
		}else{
		?>
		...
		<div class="table" style="grid-template-columns: 100px auto auto auto auto auto auto 200px;">
			...
			<?php
			$rows=$db->select("select a.*,b.artist_name,c.album_title from videos a left join artists b on b.id=a.artist_id left join albums c on c.id=a.album_id where a.media_type='video' order by video_title");
			for($i=0; $i<count($rows); $i++)
			{
			?>
				<div class="td" style="text-align: center;"><?=($i+1)?></div>
				<div class="td"><?php echo $rows[$i]['video_title']; ?></div>
				<div class="td"><?php echo $rows[$i]['video_description']; ?></div>
				...
				<div class="td" style="text-align: center;"><a href="edit-video.php?id=<?=$rows[$i]['id']?>" class="action-item">Edit</a href=""> | <a href="javascript:void(0);" id="<?=$rows[$i]['id']?>"  class="action-item delete red">Delete</a href=""></div>
			<?php
			}
			?>
