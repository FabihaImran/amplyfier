...
<body>
  ...
  <?php include("header.php"); ?>
    ...
    <div class="container">
            ...
            <?php
            $artist=$db->select("select * from albums where id=".$_GET['aid']);
            ?>
         <h1><?=$artist[0]['album_title']?> Songs</h1>



        <div class="video-grid">
            <?php
            $rows=$db->select("select a.*,b.artist_name,c.album_title from videos a left join artists b on b.id=a.artist_id left join albums c on c.id=a.album_id where a.album_id=".$_GET['aid']." order by ts desc");
